...
const AddShows = () => {
  ...
  const handleAddShow = async () => {
    ...
    const showsInput = Object.keys(dateTimeSelection).map((date) => ({
      date,
      time: dateTimeSelection[date],
    }));
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "An error occurred while adding the show."
      );
    }
  };
        ...
        <div className="group flex flex-wrap gap-4 mt-4 w-max">
          {nowPlayingMovies.map((movie)=>(
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover: not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={()=>setSelectedMovie(movie.id)}>
              ...
              <img src={movie.poster_path?.startsWith('/') ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : movie.poster_path} alt="" className="w-full object-cover brightness-90"/>
              <div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full absolut bottom-0 left-0">
            ...
            {Object.entries(dateTimeSelection).map(([date,times])=>(
              <li key={date}>
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                  {times.map((time)=>(
                    <div key={time} className="border border-primary px-2 py-1 flex itmes-center rounded">
                      <span>{time}</span>
                      <DeleteIcon onClick={()=>
